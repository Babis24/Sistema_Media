import { Client } from 'pg'; // Importa a classe Client do módulo pg para interagir com PostgreSQL
import * as readlineSync from 'readline-sync'; // Importa o módulo readline-sync para coletar entrada do usuário no terminal
...
async function main() {
    ...
    try {
        ...
        const notasPorMateria: { [key: string]: number[] } = {}; // Objeto para armazenar as notas de cada matéria
        const mediasPorMateria: { [key: string]: number } = {}; // Objeto para armazenar as médias de cada matéria
        ...
        for (const materia of materias) {
            console.log(`\n--- Notas de ${materia} ---`);
            const notas: number[] = []; // Array para armazenar as 8 notas da matéria atual
